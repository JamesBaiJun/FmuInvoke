...
namespace Femyou
{
    class ModelImpl : IModel
    {
        ...
        {
            FmuPath = fmuPath;
            TmpFolder = Path.Combine(Path.GetTempPath(), nameof(Femyou), Path.GetFileName(FmuPath), DateTime.Now.ToString("HHmmss"));
            try
            {
                // É¾³ýÁÙÊ±ÎÄ¼þ
                Directory.Delete(Path.Combine(Path.GetTempPath(), nameof(Femyou)), true);
            }
            catch (Exception)
            {
            }
        ...
        private static Library CoSimulation;
